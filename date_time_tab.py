...
from PyQt6.QtCore import QTimer, QDateTime, Qt, QDate, QEvent
...
class DateTimeApp(QWidget):
    def __init__(self):
        ...
        # تثبيت فلتر الأحداث لتعطيل عجلة الماوس على العناصر المحددة
    ...
    def eventFilter(self, watched, event):
        """
        هذه الدالة تعترض الأحداث قبل وصولها للعناصر
        """
        # نتحقق إذا كان الحدث هو "تمرير عجلة الماوس"
        # وإذا كان العنصر المراقب هو من نوع QSpinBox أو QComboBox
        if event.type() == QEvent.Type.Wheel and isinstance(watched, (QSpinBox, QComboBox)):
            # إذا تحقق الشرطان، نتجاهل الحدث ونمنعه من الوصول للعنصر
            # القيمة "True" تعني أن الحدث قد تم التعامل معه
            return True
        
        # بالنسبة لجميع الحالات الأخرى (مثل تمرير الماوس على الخلفية أو أي عنصر آخر)
        # نسمح للحدث بالمرور للمعالجة الافتراضية، مما يسمح للـ ScrollArea بالتمرير
